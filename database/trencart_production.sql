-- =====================================================
-- TRENCART - PRODUCTION DATABASE SCHEMA
-- Version: 2.0 (includes size variants)
-- Charset: utf8mb4 / utf8mb4_unicode_ci
-- Safe to run fresh: uses IF NOT EXISTS throughout
-- =====================================================
...
-- Database
CREATE DATABASE IF NOT EXISTS notechin_trencart CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE notechin_trencart;
...
-- Shop profiles (legacy; main shop data lives in shops table)
...
-- Login attempt log (security / rate limiting)
...
-- Product categories (supports parent → child hierarchy)
...
CREATE TABLE IF NOT EXISTS products (
    ...
    stock_quantity      INT           DEFAULT 0,          -- sum of all size variant stocks when sizes exist
    low_stock_threshold INT           DEFAULT 10,
    color               VARCHAR(50),
    size                VARCHAR(50),                      -- comma-joined size label summary e.g. "S,M,XL"
...
-- Product size variants (S, M, L, XL, XXL, Free Size, custom …)
CREATE TABLE IF NOT EXISTS product_sizes (
    size_id          INT           PRIMARY KEY AUTO_INCREMENT,
    product_id       INT           NOT NULL,
    size_label       VARCHAR(30)   NOT NULL,          -- e.g. 'S', 'XL', 'Free Size'
    stock_quantity   INT           NOT NULL DEFAULT 0,
    price_adjustment DECIMAL(10,2) NOT NULL DEFAULT 0.00,  -- added to base price; 0 = no change
...
-- Product images (multiple per product, one marked primary)
...
-- Order items (one row per product per order; includes size selection)
CREATE TABLE IF NOT EXISTS order_items (
    ...
    product_id          INT           NOT NULL,
    product_name        VARCHAR(255)  NOT NULL,
    selected_size       VARCHAR(30)   NULL,            -- size chosen by customer (NULL if no sizes)
...
-- Shopping cart (server-side; app uses localStorage primarily)
...
-- =============================================================================
-- SECTION 3: ADMIN / FINANCE TABLES
-- =============================================================================

-- Weekly / monthly shop payment settlements
...
SELECT 'admin@trencart.com', 'Super Admin', '9999999999', 'admin', 1, 1
FROM DUAL
...
SELECT u.user_id, 'super_admin', '{"all": true}'
FROM users u
WHERE u.email = 'admin@trencart.com'
  AND NOT EXISTS (
      SELECT 1 FROM admin_profiles ap
      INNER JOIN users u2 ON ap.user_id = u2.user_id
      WHERE u2.email = 'admin@trencart.com'
  );

-- Default categories (runs only when categories table is empty)
...
-- Sub-categories (runs only when no sub-categories exist)
INSERT INTO categories (category_name, parent_category_id, category_description, display_order)
SELECT * FROM (
    SELECT 'Silk Sarees',       (SELECT category_id FROM categories WHERE category_name='Sarees'          LIMIT 1), 'Pure silk sarees',            1 UNION ALL
    ...
    SELECT 'Designer Sarees',   (SELECT category_id FROM categories WHERE category_name='Sarees'          LIMIT 1), 'Designer and party wear',     4 UNION ALL
...
SELECT * FROM (
    ...
    SELECT 'min_payout',                     '500',            'number',       'Minimum payout amount (INR)'                 UNION ALL
...
SELECT
    u.user_id, u.email, u.full_name, u.phone,
    u.is_verified, u.is_active, u.created_at, u.last_login,
    sp.shop_id, sp.shop_name, sp.shop_slug, sp.description,
    sp.shop_image, sp.rating, sp.total_reviews, sp.is_verified AS shop_verified
FROM users u
LEFT JOIN shop_profiles sp ON u.user_id = sp.user_id
WHERE u.user_type = 'shop';


-- Products with shop + category info + primary image
CREATE OR REPLACE VIEW products_with_shop AS
SELECT
    p.*,
    s.shop_name, s.shop_status,
    c.category_name,
...
-- Order details with customer + shop info
...
-- Admin: per-shop revenue summary
...
SELECT
    ...
    COALESCE((
        ...
        WHERE oi.shop_id = s.shop_id AND DATE(o.order_date) = CURDATE()
    ), 0) AS today_sales,
    COALESCE((
        SELECT SUM(oi.subtotal) FROM order_items oi
        INNER JOIN orders o ON oi.order_id = o.order_id
    ...
    COALESCE((
        SELECT SUM(paid_amount) FROM shop_payments
        WHERE shop_id = s.shop_id AND payment_status = 'paid'
    ), 0) AS total_paid,
    COALESCE((
        SELECT SUM(payable_amount - paid_amount) FROM shop_payments
        WHERE shop_id = s.shop_id AND payment_status != 'paid'
    ), 0) AS total_pending
FROM shops s
...
-- Increment shop product count on new product
...
-- Decrement shop product count on delete
...
-- Recalculate product rating after new approved review
...
-- Append a row to order_status_history whenever order status changes
...
-- =============================================================================
-- DONE  — TrenCart Production Schema v2.0
-- =============================================================================
--
-- Tables (24):
...
--   ECOMMERCE : shops, categories, products, product_sizes, product_images,
--               orders, order_items, order_status_history,
--               cart, wishlist, product_reviews, notifications
--   ADMIN     : shop_payments, admin_activity_log, platform_settings
--
-- Views (7)  : customer_users, shop_users, products_with_shop, order_details,
--              admin_shop_revenue, admin_daily_revenue, admin_order_summary
-- Triggers (4): after_product_insert/delete, after_review_insert,
--               after_order_status_update
-- Procedures (2): cleanup_expired_otps, cleanup_expired_sessions
--
-- Default admin: admin@trencart.com  (login via OTP)
-- Default categories: 6 parent + 7 sub-categories
-- =============================================================================
